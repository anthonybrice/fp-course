...
lift3 ::
  ...
  -> f c
  -> f d
lift3 f xs ys zs = pure f <*> xs <*> ys <*> zs
...
lift4 ::
  ...
  -> f d
  -> f e
lift4 f xs ys zs ws = pure f <*> xs <*> ys <*> zs <*> ws
...
replicateA ::
  ...
  -> f a
  -> f (List a)
replicateA x xs = sequence $ replicate x xs
...
filtering ::
  ...
  -> List a
  -> f (List a)
filtering p xs = _helpMe filter p xs
